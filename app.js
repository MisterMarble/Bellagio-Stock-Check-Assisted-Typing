...
function materialNames(){
  return Array.from(new Set(state.rowIndex.map(r=>r.material_name).filter(Boolean))).sort((a,b)=>a.localeCompare(b));
}
function materialMatches(q){
  const needle = (q||'').trim().toLowerCase();
  if(needle.length < 2) return [];
  return materialNames().filter(name=>name.toLowerCase().includes(needle)).slice(0,8);
}
...
function renderMaterialSuggestions(){
  ...
  const box = document.getElementById('materialSuggestions');
  if(!input || !box) return;
  const matches = materialMatches(input.value);
  box.innerHTML = '';
  if(!matches.length){
    box.hidden = true;
    return;
...
function openAddModal(){ document.getElementById('addSlabModal').showModal(); }
